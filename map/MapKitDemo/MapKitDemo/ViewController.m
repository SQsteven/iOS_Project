...
{
    //点击在view上的位置
    CGPoint touchPos = [tap locationInView:tap.view];
    //转换坐标:
//    UIView之间转换坐标
//    CALayer之间转换坐标
//    地图的坐标转换：不同类型的坐标
    CLLocationCoordinate2D convertCoor = [_mapView convertPoint:touchPos toCoordinateFromView:tap.view];
    ...
    for (int i=0; i<coorCnt; ++i) {
        coors[i].latitude = convertCoor.latitude + random()%100*1.0f*0.0001f;
        coors[i].longitude = convertCoor.longitude + random()%100*1.0f*0.00001f;;
    }
